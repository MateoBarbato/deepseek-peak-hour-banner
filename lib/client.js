...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Human distance such as `1 h 12 min`, or `2 d 12 h` across the weekend gap.
		 * Rounds up, so the last minute before a boundary reads `1 min` instead of
		 * `0 min`.
		 */
		function formatDistance(target, now) {
			const total = Math.max(0, Math.ceil((target - now) / MINUTE_MS));
		...
		/** Spanish weekday abbreviations, indexed by `Date.getDay()`. */
		...
		/**
		 * Local wall clock for one instant, prefixed with the weekday when it is not
		 * today. The weekend gap puts the next window two days out, and `22:00 local`
		 * on its own reads as tonight — which then contradicts a 60-hour countdown.
		 */
		function formatLocalStamp(ms, now) {
			const clock = `${formatLocalClock(ms)} local`;
			if (sameLocalDay(ms, now)) return clock;
			return `${WEEKDAY_SHORT[new Date(ms).getDay()]} ${clock}`;
		}
		//#endregion
		...
		function usePeakState() {
			...
			return {
				...
				local: transition === undefined ? "" : formatLocalStamp(transition.at, now),
				when: transition === undefined ? "" : `${formatUtcClock(transition.at)} UTC / ${formatLocalStamp(transition.at, now)}`
			};
		}
		/**
		...
		function PeakHourStats() {
			...
			if (peak) return null;
			// `local` already carries its own "local" suffix and, when needed, the weekday.
			const text = distance === "" ? "Fuera de hora pico" : `Fuera de hora pico · próxima ${local} (en ${distance})`;
			return react.createElement(
				"div",
